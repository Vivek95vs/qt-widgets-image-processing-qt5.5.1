...
//            Log_Data[i*ImageSize + j] = max(Log_Data[i*ImageSize + j],1);
            ...
            LogImage = -logf(min(max(Log_Data[i*ImageSize + j]/vMax, 0), 1));//converting from 0 to 1
    ...
    qDebug()<<"dimension and Tiles"<<ImageSize<<numTiles[0]<<numTiles[1]<<dimTile[0]<<dimTile[1];
...
//            qDebug()<<"Index"<<index;
            double val1= index.toDouble();
            double alpha=val1;
//            qDebug()<<"alpha"<<alpha;
            float hconst,vmax,temp,val;
            ...
            qDebug()<<"imgrow"<<imgRow<<dimTile[0];

        }
        imgCol = imgCol /*+ dimTile[1]*/;
        qDebug()<<"imgCol"<<imgCol<<dimTile[1];
    }
...
//void ImageEnhancementNew::receivedatasig(QString &arg3)
//{
//    qDebug()<<"value"<<arg3;
//}
