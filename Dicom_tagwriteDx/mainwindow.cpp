...
void MainWindow::on_PB_load_clicked()
{
    ...
    fread(image,1,w*h*sizeof(unsigned char),Img);
...
//    DOB.resize(10);
    ...
    gdcm::Attribute<0x0010,0x1010,VR::AS,VM::VM1>PatientAge={"25"};
    ...
    gdcm::Attribute<0x0008,0x0060,VR::CS,VM::VM1>Modality={"DX"};
    ...
    gdcm::Attribute<0x0028,0x0030,VR::DS,VM::VM2>ImgerPixelSpacing={0.319,0.319};
...
//    RTimagePosY=h/2;   //RTimagePosY = (radSize*pixspacing) - isoCentreY;
    ...
    gdcm::Attribute<0x3002,0x0022,VR::DS,VM::VM1>RadiationMachineSAD={1000};
    ds.Insert(RadiationMachineSAD.GetAsDataElement());

    gdcm::Attribute<0x3002,0x0026,VR::DS,VM::VM1>RTImageSID={1500};
    ds.Insert(RTImageSID.GetAsDataElement());

    gdcm::MediaStorage msUID=gdcm::MediaStorage::DigitalXRayImageStorageForProcessing;
    ...
    gdcm::MediaStorage ms1 = gdcm::MediaStorage::DigitalXRayImageStorageForProcessing;
...
//        image1.SetSpacing(spacing1);
        ...
        pixeldata.SetByteValue( (char*)buffer, w*h*sizeof(unsigned char) );
        image1.SetDataElement( pixeldata );

        QString path = ("D:\\DxWrite");
        qDebug()<<"path"<<path;
        writer.SetFileName((path+"\\Proj_"+QString::number(count+1)+".dcm").toStdString().c_str());
        writer.SetFile(file);
        if( !writer.Write())
        {
