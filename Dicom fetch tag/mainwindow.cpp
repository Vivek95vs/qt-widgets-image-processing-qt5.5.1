...
void WriteMetaData(std::ofstream& outFile, vtkDICOMMetaData* metaData) {


//    vtkSmartPointer<vtkDICOMDictionary>dicomDictionary=vtkSmartPointer<vtkDICOMDictionary>::New();

    // Get the number of attributes
    int numberOfAttributes = metaData->GetNumberOfDataElements();

    qDebug()<<"total element"<<numberOfAttributes;
    for (int i = 0; i < numberOfAttributes; ++i) {
        vtkDICOMTag tag = vtkDICOMTag(i);
        metaData->GetAttributeValue(i);
//        std::string tagName = dicomDictionary->GetAttributeName(tag);
//        std::string tagValue = metaData->GetAttributeValueAsString(tag);

        // Write the tag and its value to the output file
        outFile << "Tag (" << std::hex << std::setw(4) << std::setfill('0')
                << tag.GetGroup() << ", " << std::setw(4) << tag.GetElement()
                << ") [" << tagName << "]: " << tagValue << std::endl;
    }
}

void MainWindow::on_pushButton_clicked()
{
    const char* dicomFilePath = "D:\\Dicom tag fetch\\CT1.2.840.113619.2.417.3.2831165742.676.1702348178.107.1.dcm";
    const char* outputFilePath = "D:\\Dicom tag fetch\\Dicom Tag.txt";

    // Create VTK DICOM reader
    vtkSmartPointer<vtkDICOMReader> reader = vtkSmartPointer<vtkDICOMReader>::New();
    reader->SetFileName(dicomFilePath);
    reader->Update();
    ...
    outFile.close();
    reader->Delete();
    std::cout << "All tags written to " << outputFilePath << std::endl;
