...
//    y=event->scenePos();
    ...
    if(selectedTool==1)
    {
        ...
        rect->setRect(newrect);
            QSize size(3072, 3072);

//        QImage image(newrect.size(),QImage::Format_Grayscale8);
        QImage image(size,QImage::Format_Grayscale8);
        image.fill(Qt::transparent);
        QPainter painter(&image);
        this->render(&painter, QRectF(0, 0, 3072, 3072),QRectF(0, 0, 3072, 3072) );
...
//        QPainterPath path=mypath->path();
        ...
        QPainter painter(&image);
        this->render(&painter, QRectF(0, 0, 3072, 3072),QRectF(0, 0, 3072, 3072) );
        image.save("example_image_FH.png");
...
//        rect->setFlag(QGraphicsRectItem::ItemIsMovable);
        ...
        qDebug()<<"Rect value emitted"<<rect->x();
        emit emitrect(rect);
